...
const Cloudinary = ({ 
  ...
  reset, 
  disabled, 
  buttonText = "Subir Archivo", // Cambiado a "Archivo" en lugar de "Imagen"
  existingImage = null,
  showPreview = true,
  onRemoveImage
}) => {
    ...
    const deleteFileFromCloudinary = async (fileUrl) => {
        ...
        try {
            ...
            // Llamar a tu endpoint del backend para eliminar el archivo
    ...
    const uploadFile = async (e) => {
        ...
        if (!allowedTypes.includes(file.type)) {
            alert(`Tipo de archivo no permitido: ${file.type}. Formatos aceptados: PDF, DOC, DOCX, XLS, XLSX, JPG, PNG, GIF, BMP, TIFF, WEBP, SVG`);
            return;
        }
        ...
        try {
            ...
            onUploadComplete(uploadedFile.secure_url);
        } catch (error) {
    ...
    const handleRemoveFile = async () => {
        if (image) {
            ...
            const deleteResult = await deleteFileFromCloudinary(image);
            
            if (!deleteResult.success) {
                console.warn('No se pudo eliminar el archivo de Cloudinary, pero se eliminó localmente');
            }
        }
    ...
    const getFileIcon = (url) => {
        ...
        if (['svg'].includes(extension)) return '⚡';
        if (['tiff', 'tif'].includes(extension)) return '📷';
        ...
        <div className="cloudinary">
            {showUploadButton && !image && (
                <div className="upload-section">
                    <label htmlFor="file" className="labelFile" style={{ opacity: disabled ? 0.6 : 1 }}>
                        ...
                        <p>{buttonText}</p>
                        <small>Formatos: PDF, DOC, DOCX, XLS, XLSX, JPG, PNG, GIF, BMP, TIFF, WEBP, SVG</small>
                    </label>
                    <input
                        id="file"
                        type="file"
                        onChange={uploadFile}
                        className="input-cloudinary"
                        disabled={disabled}
                        accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.gif,.bmp,.tiff,.tif,.webp,.svg"
                    />
                </div>
            )}
                ...
                <div className="uploaded-preview">
                    <div className="preview-header">
                        ...
                        <div className="file-info">
                            ...
                            <span className="file-type">({getFileType(image)})</span>
                        </div>
                    </div>
                    
                    {isImageFile(image) ? (
                        <img src={image} alt="Vista previa" className='uploaded-image' />
                    ) : (
                        <div className="file-preview">
                            <span className="file-type-icon">{getFileIcon(image)}</span>
                            <p>Archivo {getFileType(image)} subido</p>
                            <small>{getFileName(image)}</small>
                        </div>
                    )}
            ...
            {image && !loading && !showPreview && (
                <div className="minimal-preview">
                    ...
                    <span className="minimal-text">Archivo {getFileType(image)} adjuntado</span>
                    <button 
                        onClick={handleRemoveFile} 
